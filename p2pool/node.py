...
class P2PNode(p2p.Node):
    ...
    __slots__ = ('node', 'shared_share_hashes', '__weakref__')
    ...
    def __init__(self, node, **kwargs):
        ...
        p2p.Node.__init__(self,
            ...
            # transactions from getblocktemplate
            mining_txs_var=node.mining_txs_var,
            # transactions sent to miners
            mining2_txs_var=node.mining2_txs_var,
        **kwargs)

    def handle_shares(self, shares, peer):
        if len(shares) > 5:
            print('Processing %i shares from %s...' %
                    (len(shares), '%s:%i' % peer.addr if
                            peer is not None else None))

        new_count = 0
        all_new_txs = {}
        for share, new_txs in shares:
            if new_txs is not None:
                all_new_txs.update((bitcoin_data.hash256(
                    bitcoin_data.tx_type.pack(new_tx)), new_tx) for
                        new_tx in new_txs)
        ...
        if len(shares) > 5:
            print('... done processing %i shares. New: %i Have: %i/~%i' % (
                len(shares), new_count, len(self.node.tracker.items),
                2 * self.node.net.CHAIN_LENGTH))
    ...
    def handle_get_shares(self, hashes, parents, stops, peer):
        ...
        for share_hash in hashes:
            for share in self.node.tracker.get_chain(
                    share_hash, min(parents + 1,
                                    self.node.tracker.get_height(share_hash))):
    ...
    def broadcast_share(self, share_hash):
        shares = []
        for share in self.node.tracker.get_chain(
                share_hash, min(5, self.node.tracker.get_height(share_hash))):
        ...
        for peer in self.peers.values():
            peer.sendShares([share for share in shares if
                    share.peer_addr != peer.addr], self.node.tracker,
                self.node.known_txs_var.value, include_txs_with=[share_hash])

    def start(self):
        p2p.Node.start(self)

        self.shared_share_hashes = set(self.node.tracker.items)
        self.node.tracker.removed.watch_weakref(self,
                lambda self, share: self.shared_share_hashes.discard(
                    share.hash))
        ...
        def download_shares():
            while True:
                ...
                try:
                    shares = yield peer.get_shares(
                        hashes=[share_hash],
                        # randomize parents so that we eventually get
                        # past a too large block of shares
                        parents=random.randrange(500),
                        stops=list(set(self.node.tracker.heads) | set(
                            self.node.tracker.get_nth_parent_hash(
                                head, min(max(0,
                                    self.node.tracker.get_height_and_last(
                                        head)[0] - 1), 10)) for \
                                                head in self.node.tracker.heads
                        ))[:100],
                    )
                ...
                if not shares:
                    # sleep so we don't keep rerequesting
                    # the same share nobody has
                    yield deferral.sleep(1)
                    continue
                self.handle_shares([(share, []) for share in shares], peer)
        download_shares()
        ...
        def _(share):
            ...
            def spread():
                if (self.node.get_height_rel_highest(
                    share.header['previous_block']) > -5 or
                        self.node.bitcoind_work.value['previous_block'] in
                            [share.header['previous_block'], share.header_hash]):
...
class Node(object):
    ...
    __slots__ = ('factory', 'bitcoind', 'net', 'cur_share_ver', 'known_txs_var',
                 'mining_txs_var', 'mining2_txs_var', 'best_share_var',
                 'desired_var', 'txidcache', 'feecache', 'feefifo', 'punish',
                 'tracker', 'p2p_node', 'stop', 'bitcoind_work',
                 'best_block_header', 'handle_header', 'get_height_rel_highest',
                 'get_height')

    def __init__(self, factory, bitcoind, shares, known_verified_share_hashes,
                 net):
    ...
    def start(self):
        ...
        self.bitcoind_work = variable.Variable(
                (yield helper.getwork(self.bitcoind)))
            ...
            while stop_signal.times == 0:
                ...
                try:
                    self.bitcoind_work.set((yield helper.getwork(
                        ...
                        self.txidcache, self.feecache, self.feefifo,
                        self.known_txs_var.value)))
                    self.check_and_purge_txs()
                except:
                    log.err()
                yield defer.DeferredList(
                        [flag, deferral.sleep(15)], fireOnOneCallback=True)
        work_poller()
        ...
        yield deferral.retry(
                'Error while requesting best block header:')(poll_header)()

        # BEST SHARE

        self.get_height_rel_highest, self.get_height = yield \
                height_tracker.get_height_funcs(self.bitcoind, self.factory,
                        lambda: self.bitcoind_work.value['previous_block'],
                        self.net)
        ...
        def _(_=None):
            new_mining_txs = dict(zip(
                self.bitcoind_work.value['transaction_hashes'],
                self.bitcoind_work.value['transactions']))
            added_known_txs = {hsh:tx for
                    hsh,tx in new_mining_txs.items() if
                        not hsh in self.known_txs_var.value}
        ...
        def _(share):
            ...
            if block is None:
                print('GOT INCOMPLETE BLOCK FROM PEER! %s bitcoin: %s%064x' % (
                    p2pool_data.format_hash(share.hash),
                    self.net.PARENT.BLOCK_EXPLORER_URL_PREFIX,
                    share.header_hash), file=sys.stderr)
                return
        ...
        def forget_old_txs():
            ...
            for share in self.tracker.get_chain(self.best_share_var.value,
                    min(120, self.tracker.get_height(self.best_share_var.value))):
                if share.VERSION >= 34:
                    continue
    ...
    def set_best_share(self):
        ...
        best, desired, decorated_heads, bad_peer_addresses, self.punish = \
                self.tracker.think(self.get_height_rel_highest, self.get_height,
                        self.bitcoind_work.value['previous_block'],
                        self.bitcoind_work.value['bits'],
                        self.known_txs_var.value, self.feecache)
        # need to reissue work with lower difficulty
        if self.punish and not oldpunish \
                and best == self.best_share_var.value:
            # triggers wb.new_work_event to reissue work
            self.best_share_var.changed.happened(best)
    ...
    def get_current_txouts(self):
        return p2pool_data.get_expected_payouts(
                ...
                self.bitcoind_work.value['bits'].target,
                self.bitcoind_work.value['subsidy'], self.net)
    ...
    def clean_tracker(self):
        ...
        if decorated_heads:
            for i in range(1000):
                ...
                for share_hash, tail in self.tracker.heads.items():
                    if share_hash in [head_hash for
                            score, head_hash in decorated_heads[-5:]]:
                        #print 1
                        continue
                    if self.tracker.items[share_hash].time_seen \
                            > time.time() - 300:
                        #print 2
                        continue
                    if share_hash not in self.tracker.verified.items \
                            and max(self.tracker.items[
                                after_tail_hash].time_seen for
                                    after_tail_hash in
                                        self.tracker.reverse.get(tail)) \
                            > time.time() - 120: # XXX stupid
                        #print 3
                        continue
        ...
        for i in range(1000):
            ...
            for tail, heads in self.tracker.tails.items():
                if min(self.tracker.get_height(head) for
                        head in heads) \
            ...
            for aftertail in to_remove:
                if self.tracker.items[aftertail].previous_hash \
                        not in self.tracker.tails:
                    print("erk %s %s" % (
                        aftertail, self.tracker.items[aftertail].previous_hash))
                    continue
