...
class Error(Exception):
    ...
    def __str__(self):
        return '%i %s%s' % (self.code, self.message,
                            ' %r' % self.data if self.data is not None else '')
...
class Proxy(object):
    ...
    def __getattr__(self, attr):
        if attr.startswith('rpc_'):
            return lambda *params: self._func(
                    '.'.join(self._services + [attr[len('rpc_'):]]), params)
        ...
        try:
            try:
                ...
                if 'result' in req or 'error' in req:
                    response_handler(req['id'],
                            req['result'] if 'error' not in req \
                                    or req['error'] is None else
                        failure.Failure(Error_for_code(req['error']['code'])
                            (req['error']['message'],
                             req['error'].get('data', None))))
                ...
                method_meth = getattr(provider,
                        'rpc_%s' % method.split('.')[-1], None)
...
def _http_do(url, headers, timeout, method, params):
    ...
    try:
        ...
        try:
            resp = json.loads(e.response.decode('ascii'))
        except:
            raise e
    else:
    ...
    if 'error' in resp and resp['error'] is not None:
        raise Error_for_code(resp['error']['code'])(resp['error']['message'],
                resp['error'].get('data', None))
    defer.returnValue(resp['result'])

HTTPProxy = lambda url, headers={}, timeout=5: Proxy(
        lambda method, params: _http_do(url, headers, timeout, method, params))
...
class HTTPServer(deferred_resource.DeferredResource):
    ...
    def render_POST(self, request):
        data = yield _handle(request.content.read(), self._provider,
                             preargs=[request])
...
class LineBasedPeer(basic.LineOnlyReceiver):
    ...
    __slots__ = ('_matcher', 'other')

    delimiter = b'\n'

    def __init__(self):
        #basic.LineOnlyReceiver.__init__(self)
        self._matcher = deferral.GenericDeferrer(max_id=2**30,
                func=lambda id, method, params: self.sendLine(
                    json.dumps({'jsonrpc': '2.0', 'method': method,
                                'params': params, 'id': id}).encode('ascii')))
        self.other = Proxy(self._matcher)

    def lineReceived(self, line):
        _handle(line, self, response_handler=self._matcher.got_response
                ).addCallback(lambda line2: self.sendLine(
                    line2) if line2 is not None else None)
