...
class FirstByteSwitchProtocol(protocol.Protocol):
    ...
    def dataReceived(self, data):
        if self.p is None:
            if not data: return
            serverfactory = self.factory.first_byte_to_serverfactory.get(
                    data[0], self.factory.default_serverfactory)
...
class FirstByteSwitchFactory(protocol.ServerFactory):
    ...
    __slots__ = ('first_byte_to_serverfactory', 'default_serverfactory')
    ...
    def startFactory(self):
        for f in list(self.first_byte_to_serverfactory.values()) + \
                [self.default_serverfactory]:
            f.doStart()
