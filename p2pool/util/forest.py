...
class TrackerSkipList(skiplist.SkipList):
    ...
    def __init__(self, tracker):
        ...
        self.tracker.removed.watch_weakref(
                self, lambda self, item: self.forget_item(item.hash))

    def previous(self, element):
        return self.tracker._delta_type.from_element(
                self.tracker.items[element]).tail
...
class DistanceSkipList(TrackerSkipList):
    ...
    def combine_deltas(self, hash1, hash2):
        # hash1 = (from_hash1, dist1, to_hash1)
        # hash2 = (from_hash2, dist2, to_hash2)
        if hash1[2] != hash2[0]:
...
def get_attributedelta_type(attrs):
    # attrs: {name: func}
    class ProtoAttributeDelta(object):
        ...
        def from_element(cls, item):
            return cls(item.hash, item.previous_hash, **dict((k, v(item)) for
                    k, v in attrs.items()))
        ...
        def __add__(self, other):
            ...
            return self.__class__(self.head, other.tail,
                    **dict((k, getattr(self, k) + getattr(other, k)) for
                            k in attrs))

        def __sub__(self, other):
            if self.head == other.head:
                return self.__class__(other.tail, self.tail,
                        **dict((k, getattr(self, k) - getattr(other, k)) for
                                k in attrs))
            elif self.tail == other.tail:
                return self.__class__(self.head, other.head,
                        **dict((k, getattr(self, k) - getattr(other, k)) for k
                            in attrs))
            else:
                raise AssertionError()
        ...
        def __repr__(self):
            return '%s(%r, %r%s)' % (
                    self.__class__, self.head, self.tail,
...
class TrackerView(object):
    ...
    __slots__ = ('_tracker', '_delta_type', '_deltas', '_reverse_deltas',
                 '_ref_generator', '_delta_refs', '_reverse_delta_refs',
                 '__weakref__')
    ...
    def __init__(self, tracker, delta_type):
        ...
        self._tracker.remove_special.watch_weakref(self,
                lambda self, item: self._handle_remove_special(item))
        self._tracker.remove_special2.watch_weakref(self,
                lambda self, item: self._handle_remove_special2(item))
        self._tracker.removed.watch_weakref(self,
                lambda self, item: self._handle_removed(item))
    ...
    def _handle_remove_special(self, item):
        ...
        # move delta refs referencing children down to this,
        # so they can be moved up in one step
        for x in list(self._reverse_deltas.get(self._reverse_delta_refs.get(
                delta.head, object()), set())):
            self.get_last(x)

        assert delta.head not in self._reverse_delta_refs, list(
                self._reverse_deltas.get(self._reverse_delta_refs.get(
                    delta.head, object()), set()))
...
class Tracker(object):
    ...
    __slots__ = ('items', 'reverse', 'heads', 'tails', 'added', 'get_last',
                 'remove_special', 'remove_special2', 'removed', 'get_height',
                 'get_nth_parent_hash', 'get_height_and_last', '_delta_type',
                 '_default_view', 'get_work')
    ...
    def remove(self, item_hash):
        ...
        if delta.head in self.heads and delta.tail in self.tails:
            ...
            heads = [x for x in self.tails[delta.tail] if self.is_child_of(
                delta.head, x)]
    ...
    def is_child_of(self, item_hash, possible_child_hash):
        ...
        return height_up >= 0 and self.get_nth_parent_hash(
                possible_child_hash, height_up) == item_hash

class SubsetTracker(Tracker):

    __slots__ = ('get_nth_parent_hash', '_subset_of', 'get_delta')

    def __init__(self, subset_of, **kwargs):
        Tracker.__init__(self, **kwargs)
        # overwrites Tracker.__init__'s
