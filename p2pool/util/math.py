...
def ierf(z):
    return find_root(
            lambda x: (erf(x) - z)/(2*math.e**(-x**2)/math.sqrt(math.pi)), 0)
...
def binomial_conf_interval(x, n, conf=0.95):
    ...
    bottom = 1 + z**2/n
    return [clip(x, (0, 1)) for x in add_to_range(
        x/n, [(topa - topb)/bottom, (topa + topb)/bottom])]
...
def format_binomial_conf(x, n, conf=0.95, f=lambda x: x):
    ...
    return '~%.1f%% (%.f-%.f%%)' % \
        (100 * f(x / n), math.floor(100 * left), math.ceil(100 * right))
...
def string_to_natural(s, alphabet=None):
    if alphabet is None:
        ...
        assert len(set(alphabet)) == len(alphabet)
        # Not sure why s can't start with the first alphabet leter,
        # passes unittest fine
        #assert not s.startswith(bytes(ord(alphabet[0])))
        return sum(alphabet.index(char) * len(alphabet)**i for
                i, char in enumerate(reversed(s)))

class RateMonitor(object):

    __slots__ = ('max_lookback_time', 'datums', 'first_timestamp')
    ...
    def get_datums_in_last(self, dt=None):
        ...
        self._prune()
        now = time.time()
        return ([datum for ts, datum in self.datums if ts > now - dt],
                min(dt, now - self.first_timestamp) if
                    self.first_timestamp is not None else 0)
