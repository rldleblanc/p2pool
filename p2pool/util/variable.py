...
class Event(object):
    ...
    __slots__ = ('observers', 'id_generator', '_once', 'times')
...
class Variable(object):
    ...
    __slots__ = ('value', 'changed', 'transitioned')
...
class VariableDict(Variable):
    ...
    def add(self, values):
        new_items = dict([item for item in values.items() if
            not item[0] in self.value or self.value[item[0]] != item[1]])
        self.value.update(values)
        self.added.happened(new_items)
        # XXX call self.changed and self.transitioned

    def remove(self, values):
        gone_items = dict([item for item in values.items() if
            item[0] in self.value])
