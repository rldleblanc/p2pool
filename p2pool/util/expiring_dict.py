...
class LinkedList(object):
    ...
    def __iter__(self):
        ...
        while cur is not self.end:
            cur2 = cur
            cur = cur.next
            # in case cur is deleted, but items inserted after are ignored
            yield cur2
...
class ExpiringDict(object):
    ...
    __slots__ = ('d', 'expiry_time', 'get_touches', 'expiry_deque',
                 '_expire_loop', '__weakref__')
    ...
    def __init__(self, expiry_time, get_touches=True):
        ...
        self_ref = weakref.ref(self,
                lambda _: expire_loop.stop() if expire_loop.running else None)
        self._expire_loop = expire_loop = deferral.RobustLoopingCall(
                lambda: self_ref().expire())
        expire_loop.start(1)
    ...
    def touch(self, key, value=_nothing):
        ...
        self.d[key] = self.expiry_deque.append(
                (time.time() + self.expiry_time, key)), new_value
        return new_value
