...
def prefix_to_hash_link(prefix, const_ending=b''):
    ...
    return dict(state=x.state, length=x.length // 8,
...
def check_hash_link(hash_link, data, const_ending=b''):
    ...
    extra = (hash_link['extra_data'] + const_ending)[
            len(hash_link['extra_data']) + len(const_ending) - extra_length:]
    assert len(extra) == extra_length
    return pack.IntType(256).unpack(hashlib.sha256(sha256.sha256(data, (
        hash_link['state'], extra, 8*hash_link['length'])).digest()).digest())
...
def load_share(share, net, peer_addr):
    ...
    if share['type'] in share_versions:
        return share_versions[share['type']](
                net, peer_addr, share_versions[share['type']
                    ].get_dynamic_types(net)['share_type'
                        ].unpack(share['contents']))
...
class BaseShare(object):
    ...
    __slots__ = ('net', 'peer_addr', 'contents', 'min_header', 'share_info',
                 'hash_link', 'merkle_link', 'hash', 'share_data', 'max_target',
                 'target', 'target_timestamp', 'previous_hash', 'new_script',
                 'desired_version', 'gentx_hash', 'header', 'pow_hash',
                 'header_hash', 'new_transaction_hashes', 'time_seen',
                 'absheight', 'abswork', 'naughty', 'timestamp', 'address',
                 'all_txs_size', 'stripped_txs_size')
    ...
    def get_dynamic_types(cls, net):
        ...
        segwit_data = ('segwit_data', pack.PossiblyNoneType(dict(
            txid_merkle_link=dict(branch=[], index=0),
            wtxid_merkle_root=2**256-1), pack.ComposedType([
        ...
        t['share_info_type'] = pack.ComposedType([
            ('share_data', pack.ComposedType([
                ('previous_share_hash', pack.PossiblyNoneType(
                    0, pack.IntType(256))),
                ...
                ('stale_info', pack.EnumType(pack.IntType(8), dict(
                    (k, {0: None, 253: 'orphan', 254: 'doa'}.get(
                        k, 'unk%i' % (k,))) for k in range(256)))),
                ('desired_version', pack.VarIntType()),
            ]))] + ([segwit_data] if \
                    is_segwit_activated(cls.VERSION, net) else []) + ([
    ...
    def generate_transaction(cls, tracker, share_data, block_target,
                             desired_timestamp, desired_target, ref_merkle_link,
                             desired_other_transaction_hashes_and_fees, net,
                             known_txs=None, last_txout_nonce=0,
                             base_subsidy=None, segwit_data=None):
        t0 = time.time()
        previous_share = tracker.items[share_data['previous_share_hash']] if \
                share_data['previous_share_hash'] is not None else None
        ...
        if height < net.TARGET_LOOKBEHIND:
            ...
            pre_target = 2**256//(net.SHARE_PERIOD*attempts_per_second) - 1 if \
                    attempts_per_second else 2**256-1
        ...
        for tx_hash, fee in desired_other_transaction_hashes_and_fees:
            if known_txs is not None:
                this_stripped_size = bitcoin_data.tx_id_type.packed_size(
                        known_txs[tx_hash])
                this_real_size     = bitcoin_data.tx_type.packed_size(
                        known_txs[tx_hash])
                this_weight        = this_real_size + 3 * this_stripped_size
            else:
                # we're just verifying someone else's share.
                # We'll calculate sizes in should_punish_reason()
                this_stripped_size = 0
                this_real_size = 0
                this_weight = 0

            if all_transaction_stripped_size + this_stripped_size + 80 + \
                    cls.gentx_size +  500 > net.BLOCK_MAX_SIZE:
                break
            if all_transaction_weight + this_weight + 4 * 80 + \
                    cls.gentx_weight + 2000 > net.BLOCK_MAX_WEIGHT:
                break
        ...
        removed_fees = [fee for tx_hash, fee in \
                desired_other_transaction_hashes_and_fees if \
                    tx_hash not in included_transactions]
        definite_fees = sum(0 if fee is None else fee for \
                tx_hash, fee in desired_other_transaction_hashes_and_fees if \
                    tx_hash in included_transactions)
        ...
        weights, total_weight, donation_weight = tracker.get_cumulative_weights(
                previous_share.share_data['previous_share_hash'] if \
                        previous_share is not None else None,
            max(0, min(height, net.REAL_CHAIN_LENGTH) - 1),
            65535 * net.SPREAD *
                bitcoin_data.target_to_average_attempts(block_target),
        )
        sum_weights = sum(weights.values())
        assert total_weight == sum_weights + donation_weight, (total_weight,
                                    sum_weights + donation_weight)

        # 99.5% goes according to weights prior to this share
        amounts = dict(
                (script, share_data['subsidy'] * (199 * weight) // \
                        (200*total_weight)) for \
                            script, weight in weights.items())
        ...
        if sum(amounts.values()) != share_data['subsidy'] \
                or any(x < 0 for x in amounts.values()):
            raise ValueError()
        ...
        if len(dests) >= 200:
            print("found %i payment dests. Antminer S9s may crash when this is close to 226." %
                    len(dests))

        segwit_activated = is_segwit_activated(cls.VERSION, net)
        if segwit_data is None and known_txs is None:
            segwit_activated = False
        if not(segwit_activated or known_txs is None) \
                and any(bitcoin_data.is_segwit_tx(known_txs[h]) for \
                    h in other_transaction_hashes):
            raise ValueError('segwit transaction included before activation')
        if segwit_activated and known_txs is not None:
            share_txs = [(known_txs[h], bitcoin_data.get_txid(
                known_txs[h]), h) for \
                    h in other_transaction_hashes]
            segwit_data = dict(
                    ...
                    wtxid_merkle_root=bitcoin_data.merkle_hash([0] + \
                            [bitcoin_data.get_wtxid(tx[0], tx[1], tx[2]) for \
                                tx in share_txs]))
        ...
        share_info = dict(
            ...
            far_share_hash=None if last is None and height < 99 \
                    else tracker.get_nth_parent_hash(
            ...
            timestamp=(math.clip(desired_timestamp, (
                        ...
                        (previous_share.timestamp + net.SHARE_PERIOD) +
                            (net.SHARE_PERIOD - 1),)) if \
                                    previous_share is not None \
                                        else desired_timestamp
                      ) if cls.VERSION < 32 else
                      max(desired_timestamp, (previous_share.timestamp + 1)) if \
                              previous_share is not None else desired_timestamp,
            absheight=((previous_share.absheight if \
                    previous_share is not None else 0) + 1) % 2**32,
            abswork=((previous_share.abswork if \
                    previous_share is not None else 0) +
        ...
        if previous_share != None \
                and desired_timestamp > previous_share.timestamp + 180:
            print("Warning: Previous share's timestamp is %i seconds old." %
                    int(desired_timestamp - previous_share.timestamp))
        ...
        if previous_share != None \
                and previous_share.timestamp > int(time.time()) + 3:
        ...
        payouts = [dict(value=amounts[addr],
                        ...
                        ) for addr in dests if \
                                amounts[addr] and addr != donation_address]
    ...
    def get_ref_hash(cls, net, share_info, ref_merkle_link):
        return pack.IntType(256).pack(bitcoin_data.check_merkle_link(
            bitcoin_data.hash256(
                cls.get_dynamic_types(net)['ref_type'].pack(dict(
    ...
    def __init__(self, net, peer_addr, contents):
        ...
        if len(self.merkle_link['branch']) > 16 \
                or (segwit_activated and len(
                    self.share_info['segwit_data']['txid_merkle_link']['branch']) > 16):
        ...
        if net.NAME == 'bitcoin' and self.absheight > 3927800 \
                and self.desired_version == 16:
        ...
        self.gentx_hash = check_hash_link(
            ...
            self.get_ref_hash(net, self.share_info,
                contents['ref_merkle_link']) + pack.IntType(64).pack(
                    self.contents['last_txout_nonce']) + pack.IntType(32).pack(0),
            self.gentx_before_refhash,
        )
        merkle_root = bitcoin_data.check_merkle_link(
                self.gentx_hash,
                self.share_info['segwit_data']['txid_merkle_link'] if \
                        segwit_activated else self.merkle_link)
    ...
    def check(self, tracker, known_txs=None, block_abs_height_func=None,
        ...
        if self.share_data['previous_share_hash'] is not None \
                and block_abs_height_func is not None:
            previous_share = tracker.items[self.share_data['previous_share_hash']]
            if tracker.get_height(
                    self.share_data['previous_share_hash']) >= self.net.CHAIN_LENGTH:
                counts = get_desired_version_counts(
                        tracker, tracker.get_nth_parent_hash(
                ...
                if type(self) is type(previous_share):
                    ...
                    # switch only valid if 60% of hashes in
                    # [self.net.CHAIN_LENGTH*9//10, self.net.CHAIN_LENGTH]
                    # for new version
                    if counts.get(self.VERSION, 0) < \
                            sum(counts.values()) * 60 // 100:
        ...
        if self.VERSION < 34:
            other_tx_hashes = [tracker.items[tracker.get_nth_parent_hash(
                self.hash, share_count)
                ].share_info['new_transaction_hashes'][tx_count] for \
                        share_count, tx_count in self.iter_transaction_hash_refs()]
        else:
            other_tx_hashes = []
        ...
        share_info, gentx, other_tx_hashes2, get_share = \
                self.generate_transaction(tracker, self.share_info['share_data'],
                        self.header['bits'].target, self.share_info['timestamp'],
                        self.share_info['bits'].target,
                        self.contents['ref_merkle_link'],
                        [(h, None) for h in other_tx_hashes], self.net,
                        known_txs=None,
            ...
            if self.share_data['previous_share_hash'] is not None \
                ...
                if missing == 0:
                    ...
                    if self.share_data['subsidy'] > max_subsidy:
                        self.naughty = 1
                        print("Excessive block reward in share! Naughty. %s" %
                                details)
        ...
        if self.share_data['previous_share_hash'] \
                and tracker.items[self.share_data['previous_share_hash']].naughty:
            ...
            self.naughty = 1 + tracker.items[self.share_data[
                'previous_share_hash']].naughty
            if self.naughty > 6:
                self.naughty = 0
        ...
        if self.VERSION < 34:
            if bitcoin_data.calculate_merkle_link(
                    [None] + other_tx_hashes, 0) != self.merkle_link:
        ...
        # saving this share's gentx size as a class variable is an ugly hack,
        # and you're welcome to hate me for doing it. But it works.
        ...
        if not self.naughty:
            ...
            print("Received naughty=%i share: diff=%.2e hash=%064x miner=%s" %
                    (self.naughty, _diff, self.hash,
                        self.address.decode('ascii')))
        return gentx # only used by as_block

    def get_other_tx_hashes(self, tracker):
        parents_needed = max(share_count for
                share_count, tx_count in self.iter_transaction_hash_refs()) if \
                    self.share_info.get('transaction_hash_refs', None) else 0
    ...
    def should_punish_reason(self, previous_block, bits, tracker, known_txs):
        ...
        if self.naughty == 1:
            return (self.naughty,
                    'naughty share (excessive block reward or otherwise would make an invalid block)')
        if self.naughty:
            return (self.naughty,
                    'descendent of naughty share                                                    ')
        if self.VERSION < 34:
        ...
        if other_txs is None:
            ...
            if self.all_txs_size + 3 * self.stripped_txs_size + 4*80 + \
                    self.gentx_weight > tracker.net.BLOCK_MAX_WEIGHT:
                return True, 'txs over block weight limit'
            if self.stripped_txs_size + 80 + self.gentx_size \
                    > tracker.net.BLOCK_MAX_SIZE:
...
class SegwitMiningShare(BaseShare):
    ...
    __slots__ = ('share_info_type', 'share_type', 'ref_type')
...
class WeightsSkipList(forest.TrackerSkipList):
    ...
    def combine_deltas(self, share1, share2):
        # share1: (share_count1, weights1, total_weight1, total_donation_weight1)
        # share2: (share_count2, weights2, total_weight2, total_donation_weight2)
    ...
    def initial_solution(self, start, args):
        # args: (max_shares, desired_weight)
        assert args[1] % 65535 == 0, divmod(args[1], 65535)
        return (0, None, 0, 0)

    def apply_delta(self, share1, share2, share_info):
        # share1: (share_count1, weights_list, total_weight1,
        #          total_donation_weight1)
        # share2: (share_count2, weights2, total_weight2,
        #          total_donation_weight2)
        # share_info: (max_shares, desired_weight)
    ...
    def judge(self, share, share_info):
        # share: (share_count, weights_list, total_weight, total_donation_weight)
        # share_info: (max_shares, desired_weight)
...
class OkayTracker(forest.Tracker):
    ...
    __slots__ = ('net', 'verified', 'get_cumulative_weights',
                 'get_height_and_last', 'heads', 'get_delta')

    def __init__(self, net):
        forest.Tracker.__init__(self, delta_type=forest.get_attributedelta_type(
                dict(forest.AttributeDelta.attrs,
        ...
        self.verified = forest.SubsetTracker(
                delta_type=forest.get_attributedelta_type(dict(
    ...
    def attempt_verify(self, share, block_abs_height_func, known_txs, feecache):
        ...
        try:
            share.check(self, known_txs, feecache=feecache,
                        block_abs_height_func=block_abs_height_func)
        except:
            log.err(None, 'Share check failed: %064x -> %064x' %
                    (share.hash, share.previous_hash if \
                            share.previous_hash is not None else 0))
            return False
        else:
            self.verified.add(share)
            return True

    def think(self, block_rel_height_func, block_abs_height_func,
              previous_block, bits, known_txs, feecache):
        desired = set()
        ...
        for head in set(self.heads) - set(self.verified.heads):
            ...
            for share in self.get_chain(head, head_height if \
                    last is None else min(5, max(
                        0, head_height - self.net.CHAIN_LENGTH))):
                ...
                if last is not None:
                    desired.add((
                        self.items[random.choice(list(self.reverse[last]))
                            ].peer_addr,
                        last,
        ...
        # try to get at least CHAIN_LENGTH height for each verified head,
        # requesting parents if needed
        for head in list(self.verified.heads):
            ...
            can = max(last_height - 1 - self.net.CHAIN_LENGTH, 0) if \
                    last_last_hash is not None else last_height
            get = min(want, can)
            ...
            for share in self.get_chain(last_hash, get):
                if not self.attempt_verify(share, block_abs_height_func,
                                           known_txs, feecache):
                    break
            if head_height < self.net.CHAIN_LENGTH and last_last_hash is not None:
                desired.add((
                    self.items[random.choice(list(
                        self.verified.reverse[last_hash]))].peer_addr,
                    last_last_hash,
                    max(x.timestamp for x in self.get_chain(
                        head, min(head_height, 5))),
                    min(x.target for x in self.get_chain(
        ...
        # decide best tree
        decorated_tails = sorted((self.score(max(
                self.verified.tails[tail_hash], key=self.verified.get_work),
            block_rel_height_func), tail_hash) for \
                    tail_hash in self.verified.tails)
        if p2pool.DEBUG:
            print('%d tails:' % len(decorated_tails))
            for score, tail_hash in decorated_tails:
                print("%s %s" % (format_hash(tail_hash), score))
        best_tail_score, best_tail = decorated_tails[-1] if \
                decorated_tails else (None, None)

        # decide best verified head
        decorated_heads = sorted(((
            self.verified.get_work(self.verified.get_nth_parent_hash(
                h, min(5, self.verified.get_height(h)))) -
            min(self.items[h].should_punish_reason(
                previous_block, bits, self, known_txs)[0], 1) *
            bitcoin_data.target_to_average_attempts(self.items[h].target),
            #self.items[h].peer_addr is None,
            -self.items[h].should_punish_reason(
                previous_block, bits, self, known_txs)[0],
        ...
        punish_aggressively = traditional_sort[-1][0][2] if \
                traditional_sort else False
        if punish_aggressively > 0:
            print("Other nodes are going to follow a share we want to punish! Time to hulk up.")

        if p2pool.DEBUG:
            print('%d heads. Top 10:' % len(decorated_heads))
            for score, head_hash in decorated_heads[-10:]:
                print('    %s %s %s' % (
                    format_hash(head_hash),
                    format_hash(self.items[head_hash].previous_hash), score))
            print("Traditional sort:")
            for score, head_hash in traditional_sort[-10:]:
                print('    %s %s %s' % (
                    format_hash(head_hash),
                    format_hash(self.items[head_hash].previous_hash), score))
        best_head_score, best = decorated_heads[-1] if \
                decorated_heads else (None, None)
        ...
        if best is not None:
            ...
            while punish > 0:
                ...
                if not punish:
                    def best_descendent(hsh, limit=20):
                        ...
                        best_kids = sorted((best_descendent(child, limit-1) for
                            child in child_hashes if
                                not self.items[child].naughty))
                        if not best_kids or limit<0:
                            # in case the only children are naughty
                            return 0, hsh
                        return (best_kids[-1][0]+1, best_kids[-1][1])
                    try:
                        gens, hsh = best_descendent(best)
                        if p2pool.DEBUG:
                            print("best_descendent went %i generations for share %s from %s" % (
                                gens, format_hash(hsh), format_hash(best)))
                        best = hsh
                        best_share = self.items[best]
                    except:
                        traceback.print_exc()
            ...
            target_cutoff = int(2**256 // \
                    (self.net.SHARE_PERIOD*best_tail_score[1] + 1) * 2 + .5) if \
                        best_tail_score[1] is not None else 2**256 - 1
        else:
        ...
        if p2pool.DEBUG:
            ...
            for peer_addr, hash, ts, targ in desired:
                print('    %s %s %s %s %s %s' % (
                    None if peer_addr is None else '%s:%i' % peer_addr,
                    format_hash(hash), math.format_dt(time.time() - ts),
                    bitcoin_data.target_to_difficulty(targ),
                    ts >= timestamp_cutoff, targ <= target_cutoff))

        return (best, [(peer_addr, hash) for peer_addr, hash, ts, targ in \
                    desired if ts >= timestamp_cutoff],
                decorated_heads, bad_peer_addresses, punish_aggressively)

    def score(self, share_hash, block_rel_height_func):
        # returns approximate lower bound on chain's hashrate in the last
        # self.net.CHAIN_LENGTH*15//16*self.net.SHARE_PERIOD time

        head_height = self.verified.get_height(share_hash)
        if head_height < self.net.CHAIN_LENGTH:
            return head_height, None

        end_point = self.verified.get_nth_parent_hash(share_hash,
                self.net.CHAIN_LENGTH*15//16)

        block_height = max(block_rel_height_func(
            share.header['previous_block']) for share in
                self.verified.get_chain(end_point, self.net.CHAIN_LENGTH // 16))

        return (self.net.CHAIN_LENGTH, self.verified.get_delta(share_hash,
                end_point).work / ((0 - block_height + 1) *
                    self.net.PARENT.BLOCK_PERIOD))
...
def update_min_protocol_version(counts, share):
    ...
    if (counts is not None) and (type(share) is NewShare) \
            and (minpver < newminpver):
        if counts.get(share.VERSION, 0) >= sum(counts.values()) * 95 // 100:
            # Reject peers running obsolete nodes
...
def get_pool_attempts_per_second(tracker, previous_share_hash, dist,
                                 min_work=False, integer=False):
    assert dist >= 2
    near = tracker.items[previous_share_hash]
    far = tracker.items[tracker.get_nth_parent_hash(
        previous_share_hash, dist - 1)]
    attempts = tracker.get_delta(near.hash, far.hash).work if \
            not min_work else tracker.get_delta(near.hash, far.hash).min_work
    ...
    for share in tracker.get_chain(share_hash, lookbehind - 1):
        ...
        if s is not None:
            res[s] = res.get(s, 0) + \
                    bitcoin_data.target_to_average_attempts(share.target)
    if rates:
        dt = tracker.items[share_hash].timestamp - \
                tracker.items[tracker.get_nth_parent_hash(
                    share_hash, lookbehind - 1)].timestamp
...
def get_user_stale_props(tracker, share_hash, lookbehind, net):
    ...
    for share in tracker.get_chain(share_hash, lookbehind - 1):
        ...
        if share.share_data['stale_info'] is not None:
            ...
            total += 1
        res[key] = stale, total
    return dict((pubkey_hash, stale/total) for \
            pubkey_hash, (stale, total) in res.items())

def get_expected_payouts(tracker, best_share_hash, block_target, subsidy, net):
    weights, total_weight, donation_weight = tracker.get_cumulative_weights(
            best_share_hash, min(tracker.get_height(best_share_hash),
                net.REAL_CHAIN_LENGTH),
            65535 * net.SPREAD * bitcoin_data.target_to_average_attempts(
                block_target))
    res = dict((script, subsidy * weight // total_weight) for \
            script, weight in weights.items())
    donation_addr = donation_script_to_address(net)
    res[donation_addr] = res.get(donation_addr, 0) + subsidy - \
            sum(res.values())
    return res
...
def get_warnings(tracker, best_share, net, bitcoind_getinfo, bitcoind_work_value):
    ...
    version_warning = getattr(net, 'VERSION_WARNING',
            lambda v: None)(bitcoind_getinfo['version'])
    if version_warning is not None:
        res.append(version_warning)

    if time.time() > bitcoind_work_value['last_update'] + 60:
        res.append('''LOST CONTACT WITH BITCOIND for %s! Check that it isn't frozen or dead!''' %
...
class ShareStore(object):
    ...
    __slots__ = ('dirname', 'filename', 'net', 'known', 'known_desired')
    ...
    def __init__(self, prefix, net, share_cb, verified_hash_cb):
        ...
        for filename in filenames:
            share_hashes, verified_hashes = known.setdefault(
                    filename, (set(), set()))
                ...
                for line in f:
                    try:
                        ...
                        if type_id == 0:
                            ...
                            raw_share = share_type.unpack(
                                    binascii.unhexlify(data_hex))
                            if raw_share['type'] >= SegwitMiningShare.VERSION:
                                continue
        ...
        # filename -> (set of share hashes, set of verified hashes)
        self.known = known
        self.known_desired = dict((k, (set(a), set(b))) for \
                k, (a, b) in known.items())
    ...
    def add_share(self, share):
        for filename, (share_hashes, verified_hashes) in self.known.items():
            ...
            filename = self._add_line(b"%i %s" % (5, binascii.hexlify(
                share_type.pack(share.as_share()))))
            share_hashes, verified_hashes = self.known.setdefault(
                    filename, (set(), set()))
            share_hashes.add(share.hash)
    ...
    def get_filenames_and_next(self):
        suffixes = sorted(int(x[len(self.filename):]) for
                x in os.listdir(self.dirname) if
                    x.startswith(self.filename)
                    and x[len(self.filename):].isdigit())
        return [os.path.join(self.dirname, self.filename + str(suffix)) for
                suffix in suffixes], os.path.join(
                        self.dirname, self.filename + (str(suffixes[-1] + 1) if
                                suffixes else str(0)))

    def forget_share(self, share_hash):
        for filename, (share_hashes, verified_hashes) \
                in self.known_desired.items():
