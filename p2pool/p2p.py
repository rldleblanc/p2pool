...
class Protocol(p2protocol.Protocol):
    ...
    __slots__ = ('node', 'incoming', 'other_version', 'connected2', 'factory',
                 'connection_lost_event', 'addr', 'timeout_delayed',
                 'get_shares', 'transport', 'remote_tx_hashes',
                 'remote_remembered_tx_size', 'remembered_txs',
                 'remembered_txs_size', 'known_txs_cache',)
    ...
    def __init__(self, node, incoming):
        p2protocol.Protocol.__init__(self, node.net.PREFIX, 8000000,
                                     node.traffic_happened)
        self.node = node
    ...
    def connectionMade(self):
        ...
        # view of peer's known_txs # not actually initially empty,
        # but sending txs instead of tx hashes won't hurt
    ...
    def packetReceived(self, command, payload2):
        try:
            ...
            print('Peer %s:%i misbehaving, will drop and ban. Reason: %s' % (
        ...
        if self.node.serverfactory.listen_port is not None:
            ...
            if host is not None:
                if ':' in host:
                    ...
                    port = int(port_str)
                if p2pool.DEBUG:
                    print('Advertising for incoming connections: %s:%i' %
                            (host, port))
                # Advertise given external IP address, just as if there were
                # another peer behind us, with that address,
                # who asked us to advertise it for them
    ...
    def handle_version(self, version, services, addr_to, addr_from, nonce,
        ...
        print("Peer %s:%s says protocol version is %s, client version %s" %
                (addr_from['address'], addr_from['port'], version,
                    sub_version.decode('ascii')))
        ...
        if nonce in self.node.peers:
            if p2pool.DEBUG:
                print('Detected duplicate connection, disconnecting from %s:%i' %
                        self.addr)
            self.disconnect()
            return
        ...
        self._stop_thread = deferral.run_repeatedly(lambda: [self.send_ping(),
        ...
        watch_id0 = self.node.known_txs_var.added.watch(
                add_to_remote_view_of_my_known_txs)
        self.connection_lost_event.watch(
                lambda: self.node.known_txs_var.added.unwatch(watch_id0))
        ...
        def remove_from_remote_view_of_my_known_txs(removed):
            if removed:
                ...
                # of "losing_tx" packets doesn't cause problems
                key = max(self.known_txs_cache) + 1 if \
                        self.known_txs_cache else 0
                #dict((h, before[h]) for h in removed)
                self.known_txs_cache[key] = removed
                reactor.callLater(20, self.known_txs_cache.pop, key)
        watch_id1 = self.node.known_txs_var.removed.watch(
                remove_from_remote_view_of_my_known_txs)
        self.connection_lost_event.watch(
                lambda: self.node.known_txs_var.removed.unwatch(watch_id1))
        ...
        def update_remote_view_of_my_known_txs(before, after):
            ...
            if p2pool.BENCH and (t1-t0) > .01:
                print("%8.3f ms for update_remote_view_of_my_known_txs" %
                        ((t1 - t0) * 1000.))
        watch_id2 = self.node.known_txs_var.transitioned.watch(
                update_remote_view_of_my_known_txs)
        self.connection_lost_event.watch(
                lambda: self.node.known_txs_var.transitioned.unwatch(watch_id2))
        ...
        def update_remote_view_of_my_mining_txs(before, after):
            ...
            if removed:
                ...
                self.remote_remembered_txs_size -= sum(100 +
                        bitcoin_data.tx_type.packed_size(before[x]) for
                            x in removed)
            if added:
                self.remote_remembered_txs_size += sum(100 +
                        bitcoin_data.tx_type.packed_size(after[x]) for
                            x in added)
                assert self.remote_remembered_txs_size \
                        <= self.max_remembered_txs_size
                fragment(self.send_remember_tx,
                         tx_hashes=[x for x in added if
                                 x in self.remote_tx_hashes],
                         txs=[after[x] for x in added if
                             x not in self.remote_tx_hashes])
            t1 = time.time()
            if p2pool.BENCH and (t1-t0) > .01:
                print("%8.3f ms for update_remote_view_of_my_mining_txs" %
                        ((t1 - t0) * 1000.))

        watch_id2 = self.node.mining_txs_var.transitioned.watch(
                update_remote_view_of_my_mining_txs)
        self.connection_lost_event.watch(
                lambda: self.node.mining_txs_var.transitioned.unwatch(
                    watch_id2))

        self.remote_remembered_txs_size += sum(100 +
                bitcoin_data.tx_type.packed_size(x) for
                    x in self.node.mining_txs_var.value.values())
        assert self.remote_remembered_txs_size <= self.max_remembered_txs_size
        fragment(self.send_remember_tx, tx_hashes=[],
    ...
    def handle_addrme(self, port):
        ...
        if host == '127.0.0.1':
            if random.random() < .8 and self.node.peers:
                # services...
                random.choice(list(self.node.peers.values())).send_addrme(port=port)
        else:
            self.node.got_addr((self.transport.getPeer().host, port),
                               self.other_services, int(time.time()))
            if random.random() < .8 and self.node.peers:
    ...
    def handle_addrs(self, addrs):
        for addr_record in addrs:
            self.node.got_addr((addr_record['address']['address'],
                addr_record['address']['port']),
                addr_record['address']['services'], min(int(time.time()),
                    addr_record['timestamp']))
            if random.random() < .8 and self.node.peers:
                random.choice(list(self.node.peers.values())).send_addrs(
                        addrs=[addr_record])
    ...
    def handle_shares(self, shares):
        ...
        for wrappedshare in shares:
            ...
            share = p2pool_data.load_share(
                    wrappedshare, self.node.net, self.addr)
                ...
                for tx_hash in share.share_info['new_transaction_hashes']:
                    if tx_hash in self.node.known_txs_var.value:
                        ...
                        for cache in self.known_txs_cache.values():
                            ...
                            print('Peer referenced unknown transaction %064x, disconnecting' %
                                    tx_hash, file=sys.stderr)
                            self.disconnect()
                            return
                    txs.append(tx)
            else:
    ...
    def sendShares(self, shares, tracker, known_txs, include_txs_with=[]):
        ...
        for share in shares:
            if share.VERSION >= 34:
                ...
                # send full transaction for every new_transaction_hash
                # that peer does not know
                for tx_hash in share.share_info['new_transaction_hashes']:
                    if not tx_hash in known_txs:
                        newset   = set(share.share_info['new_transaction_hashes'])
                        ktxset   = set(known_txs)
                        missing = newset - ktxset
                        print("Missing %i of %i transactions for broadcast" %
                                (len(missing), len(newset)))
                    assert tx_hash in known_txs, \
                            'tried to broadcast share without knowing all its new transactions'
            ...
            hashes_to_send = [x for x in tx_hashes if \
                    x not in self.node.mining_txs_var.value and x in known_txs]
            all_hashes = share.share_info['new_transaction_hashes']
            new_tx_size = sum(100 + bitcoin_data.tx_type.packed_size(
                known_txs[x]) for x in hashes_to_send)
            all_tx_size = sum(100 + bitcoin_data.tx_type.packed_size(
                known_txs[x]) for x in all_hashes)
            print("Sending a share with %i txs (%i new) totaling %i msg bytes (%i new)" %
                    (len(all_hashes), len(hashes_to_send), all_tx_size,
                     new_tx_size))
        ...
        if tx_hashes:
            ...
            fragment(self.send_remember_tx, tx_hashes=[x for
                    x in hashes_to_send if x in self.remote_tx_hashes],
                txs=[known_txs[x] for
                    x in hashes_to_send if x not in self.remote_tx_hashes])
        ...
        t1 = time.time()
        if p2pool.BENCH:
            print("%8.3f ms for %i shares in sendShares (%3.3f ms/share)" %
                    ((t1 - t0) * 1000., len(shares),
                     (t1 - t0) * 1000. / max(1, len(shares))))
    ...
    def handle_sharereq(self, id, hashes, parents, stops):
        ...
        try:
            self.send_sharereply(id=id, result='good',
    ...
    message_sharereply = pack.ComposedType([
        ('id', pack.IntType(256)),
        ('result', pack.EnumType(pack.VarIntType(), {
            0: 'good', 1: 'too long', 2: 'unk2', 3: 'unk3', 4: 'unk4',
            5: 'unk5', 6: 'unk6'})),
        ('shares', pack.ListType(p2pool_data.share_type)),
    ])
    ...
    def handle_sharereply(self, id, result, shares):
        if result == 'good':
            res = [p2pool_data.load_share(share, self.node.net, self.addr) for
                    share in shares if
                        share['type'] >= p2pool_data.Share.VERSION]
        else:
    ...
    def handle_losing_tx(self, tx_hashes):
        ...
        t1 = time.time()
        if p2pool.BENCH and (t1-t0) > .01:
            print("%8.3f ms for %i txs in handle_losing_tx (%3.3f ms/tx)" %
                    ((t1 - t0) * 1000., len(tx_hashes),
                     (t1 - t0) * 1000./ max(1, len(tx_hashes))))
    ...
    def handle_remember_tx(self, tx_hashes, txs):
        ...
        for tx_hash in tx_hashes:
            if tx_hash in self.remembered_txs:
                print('Peer referenced transaction twice, disconnecting',
                        file=sys.stderr)
                self.disconnect()
                return
            ...
            self.remembered_txs_size += 100 + \
                    bitcoin_data.tx_type.packed_size(tx)
        added_known_txs = {}
        warned = False
        for tx in txs:
            ...
            if tx_hash in self.node.known_txs_var.value \
                    and not warned and p2pool.DEBUG:
        ...
        t1 = time.time()
        if p2pool.BENCH and (t1-t0) > .01:
            print("%8.3f ms for %i txs in p2p.py:handle_remember_tx (%3.3f ms/tx)" %
                    ((t1 - t0) * 1000., len(tx_hashes),
                     ((t1 - t0) * 1000. / max(1, len(tx_hashes)))))
    ...
    def connectionLost(self, reason):
        ...
        if p2pool.DEBUG:
            print("Peer connection lost: %s %s" % (self.addr, reason))
...
class ServerFactory(protocol.ServerFactory):
    ...
    __slots__ = ('node', 'max_conns', 'conns', 'running', 'listen_port')
    ...
    def buildProtocol(self, addr):
        if sum(self.conns.values()) >= self.max_conns \
                or self.conns.get(self._host_to_ident(addr.host), 0) >= 3:
            return None
        if addr.host in self.node.bans \
                and self.node.bans[addr.host] > time.time():
            return None
        p = Protocol(self.node, True)
        p.factory = self
        if p2pool.DEBUG:
            print("Got peer connection from: %s" % addr)
        return p
    ...
    def start(self):
        ...
        deferral.retry('Error binding to P2P port:',
                traceback=False)(attempt_listen)()
...
class ClientFactory(protocol.ClientFactory):
    ...
    __slots__ = ('node', 'desired_conns', 'max_attempts', 'attempts', 'conns',
                 'running')
    ...
    def clientConnectionFailed(self, connector, reason):
        self.attempts.remove(self._host_to_ident(
    ...
    def _think(self):
        try:
            if len(self.conns) < self.desired_conns and len(self.attempts) \
                    < self.max_attempts and self.node.addr_store:
                (host, port), = self.node.get_good_peers(1)

                if self._host_to_ident(host) in self.attempts:
                    pass
                elif host in self.node.bans and self.node.bans[host] \
                        > time.time():
                    pass
                else:
...
class Node(object):
    ...
    __slots__ = ('best_share_hash_func', 'port', 'net', 'addr_store',
                 'connect_addrs', 'preferred_storage', 'known_txs_var',
                 'mining_txs_var', 'mining2_txs_var', 'advertise_ip',
                 'external_ip', 'traffic_happened', 'nonce', 'peers', 'bans',
                 'clientfactory', 'serverfactory', 'running',
                 'singleclientconnectors', '_stop_thinking')

    def __init__(self, best_share_hash_func, port, net, addr_store={},
                 connect_addrs=set(), desired_outgoing_conns=10,
                 max_outgoing_attempts=30, max_incoming_conns=50,
                 preferred_storage=1000, known_txs_var=variable.VariableDict({}),
                 mining_txs_var=variable.VariableDict({}),
                 mining2_txs_var=variable.VariableDict({}), advertise_ip=True,
                 external_ip=None):
    ...
    def start(self):
        ...
        self.singleclientconnectors = [reactor.connectTCP(
            addr, port, SingleClientFactory(self)) for
                addr, port in self.connect_addrs]
    ...
    def got_conn(self, conn):
        ...
        print('%s peer %s:%i established. p2pool version: %i %r' %
                ('Incoming connection from' if conn.incoming
                    else 'Outgoing connection to',
                 conn.addr[0], conn.addr[1],
                 conn.other_version, conn.other_sub_version.decode('ascii')))
    ...
    def lost_conn(self, conn, reason):
        ...
        print('Lost peer %s:%i - %s' %
            (conn.addr[0], conn.addr[1], reason.getErrorMessage()))

    def got_addr(self, host_port, services, timestamp):
        if host_port in self.addr_store:
            old_services, old_first_seen, old_last_seen = \
                    self.addr_store[host_port]
            self.addr_store[host_port] = (services, old_first_seen,
                                          max(old_last_seen, timestamp))
        else:
            if len(self.addr_store) < 10000:
                self.addr_store[host_port] = (services, timestamp, timestamp)
    ...
    def get_good_peers(self, max_count):
        t = time.time()
        # args: (k, (servies, first_seen, last_seen))
        return [x[0] for x in sorted(self.addr_store.items(),
            key=lambda args: -math.log(max(3600, args[1][2] - args[1][1])) /
                        math.log(max(3600, t - args[1][2])) *
                        random.expovariate(1))][:max_count]
