    ...
    def test_sharereq(self):
        class MyNode(p2p.Node):
            def __init__(self, df):
                p2p.Node.__init__(self, lambda: None, 29333,
                        networks.nets['bitcoin'], {},
                        set([('127.0.0.1', 9333)]), 0, 0, 0, 0)

                self.df = df
                self.node.node.cur_share_ver = 0
    ...
    def test_tx_limit(self):
        ...
        try:
            ...
            if not (n.sent_time <= reactor.seconds() <= n.sent_time + 1):
                raise ValueError(
                        'node did not disconnect within 1 seconds of receiving too much tx data')
            yield n.stop()
        finally:
            p2p.Protocol.max_remembered_txs_size //= 10
