...
class bitcoind(object): # can be used as p2p factory, p2p protocol, or rpc jsonrpc proxy
    def __init__(self):
        ...
        self.headers = {0x16c169477c25421250ec5d32cf9c6d38538b5de970a2355fd89: {
            ...
            'bits': bitcoin_data.FloatingInteger(bits=0x1a0513c5,
                target=0x513c50000000000000000000000000000000000000000000000),
        }}
    ...
    def __getattr__(self, name):
        ...
        if name.startswith('rpc_'):
            return lambda *args, **kwargs: failure.Failure(
                    jsonrpc.Error_for_code(-32601)('Method not found'))

    def rpc_getblocktemplate(self, param):
        print("in rpc_getblocktemplate(%s)" % param)
        if param['mode'] == 'template':
            print('template requested, passing...')
            pass
        elif param['mode'] == 'submit':
            result = param['data']
            block = bitcoin_data.block_type.unpack(binascii.unhexlify(result))
            if sum(tx_out['value'] for tx_out in block['txs'][0]['tx_outs']) \
                    != sum(tx['tx_outs'][0]['value'] for
                            tx in block['txs'][1:]) + 5000000000:
                print('invalid fee')
            if block['header']['previous_block'] != self.blocks[-1]:
                return False
            if bitcoin_data.hash256(binascii.unhexlify(result)) \
        ...
        for i in range(100):
            fee = i
            txs.append({
                'data' : bitcoin_data.tx_type.pack({
                    'version': 1,
                    'tx_ins': [{'previous_output': None, 'sequence': None,
                                'script': b'foobar' * 100}],
                    'tx_outs': [{'value': fee, 'script': b'hello!'*100}],
                    'lock_time': 0}),
                'fee': fee,
            })
        print('txs built')
        return {
            "version" : 2,
            "previousblockhash" : b'%064x' % (self.blocks[-1],),
            "transactions" : txs,
            "coinbaseaux" : {
                "flags" : b"062f503253482f"
            },
            "coinbasevalue" : 5000000000 + sum(tx['fee'] for tx in txs),
            "target" : b"0000000000000513c50000000000000000000000000000000000000000000000",
            "mintime" : 1351655621,
            "mutable" : [
                "time",
                "transactions",
...
class mm_provider(object):
    ...
    def rpc_getauxblock(self, request, result1=None, result2=None):
        if result1 is not None:
            ...
            return True
        return {
            "target" : b"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", # 2**256*2/3
            "hash" : b"2756ea0315d46dc3d8d974f34380873fc88863845ac01a658ef11bc3b368af52",
            "chainid" : 1
        }
mm_provider()
...
mynet = math.Object(
    ...
    SPREAD=3, # blocks
    IDENTIFIER=binascii.unhexlify('cca5e24ec6408b1e'),
    PREFIX=binascii.unhexlify('ad9614f6466a39cf'),
    P2P_PORT=19338,
    MIN_TARGET=2**256 - 1,
    MAX_TARGET=2**256 - 1,
    PERSIST=False,
...
class MiniNode(object):
    ...
    def start(cls, net, factory, bitcoind, peer_ports, merged_urls):
        ...
        self.n.p2p_node = node.P2PNode(self.n, port=0,
                max_incoming_conns=1000000, addr_store={},
                connect_addrs=[('127.0.0.1', peer_port) for
                    peer_port in peer_ports])
        ...
        wb = work.WorkerBridge(node=self.n,
                ...
                merged_urls=merged_urls, worker_fee=3,
                args=math.Object(donation_percentage=random.uniform(0, 10),
                    address='foo', worker_fee=3, timeaddresses=1000),
                pubkeys=main.keypool(), bitcoind=bitcoind)
        self.wb = wb
...
class Test(unittest.TestCase):
    ...
    def test_node(self):
        ...
        wb = work.WorkerBridge(node=n, my_pubkey_hash=42, donation_percentage=2,
                merged_urls=[('http://127.0.0.1:%i' %
                    (mm_port.getHost().port,), '')],
                worker_fee=3, args=math.Object(donation_percentage=2,
                    address='foo', worker_fee=3, timeaddresses=1000),
                pubkeys=main.keypool(), bitcoind=bitd)
        web_root = resource.Resource()
        worker_interface.WorkerInterface(wb).attach_to(web_root)
    ...
    def test_nodes(self):
        ...
        nodes = []
        for i in range(N):
            nodes.append((yield MiniNode.start(mynet, bitd, bitd,
                [mn.n.p2p_node.serverfactory.listen_port.getHost().port for
                    mn in nodes], [])))

        yield deferral.sleep(3)

        for i in range(SHARES):
            proxy = jsonrpc.HTTPProxy('http://127.0.0.1:' +
        ...
        web2_root = web.get_web_root(nodes[0].wb, tempfile.mkdtemp(),
                variable.Variable({'errors': '', 'version': 100000}),
                stop_event)
        web2_port = reactor.listenTCP(0, server.Site(web2_root))
        for name in web2_root.listNames() + ['web/' + x for
                x in web2_root.getChildWithDefault('web', None).listNames()]:
            if name in ['web/graph_data', 'web/share', 'web/share_data']:
                continue
            print()
            print(name)
            try:
                res = yield client.getPage('http://127.0.0.1:%i/%s' %
                        (web2_port.getHost().port, name))
            except:
                import traceback
                traceback.print_exc()
            else:
        ...
        for i, n in enumerate(nodes):
            ...
            assert n.n.tracker.verified.get_height(n.n.best_share_var.value) \
                    == SHARES, (i, n.n.tracker.verified.get_height(
                        n.n.best_share_var.value))
            assert type(n.n.tracker.items[nodes[0].n.best_share_var.value]) \
                    is (data.Share.SUCCESSOR if \
                    data.Share.SUCCESSOR is not None else data.Share)
