...
class DumbTracker(object):
    ...
    @property
    def heads(self):
        return dict((x, self.get_last(x)) for x in self.items if
                x not in self.reverse)

    @property
    def tails(self):
        return dict((x, set(y for y in self.items if
            self.get_last(y) == x and y not in self.reverse)) for
                x in self.reverse if x not in self.items)
...
def test_tracker(self):
    t = DumbTracker(self.items.values())
    ...
    for start in self.items:
        ...
        assert list(self.get_chain(start, length)) == list(
                t.get_chain(start, length))

def generate_tracker_simple(n):
    t = forest.Tracker(math.shuffled(FakeShare(
        hash=i, previous_hash=i - 1 if i > 0 else None) for i in range(n)))
    test_tracker(t)
    return t
