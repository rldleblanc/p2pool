...
def random_bytes(length):
    return bytes(random.randrange(2**8) for i in range(length))
...
class Test(unittest.TestCase):
    def test_hashlink1(self):
        for i in range(100):
            ...
            assert data.check_hash_link(x, b'') == bitcoin_data.hash256(d)
    ...
    def test_hashlink3(self):
        for i in range(100):
            ...
            assert data.check_hash_link(x, d3, d2) == bitcoin_data.hash256(
                    d + d2 + d3)
    ...
    def test_skiplist(self):
        ...
        for i in range(200):
            t.add(test_forest.FakeShare(hash=i,
                previous_hash=i - 1 if i > 0 else None, new_script=i,
                share_data=dict(donation=1234), target=2**249, address='abc'))
        for i in range(200):
