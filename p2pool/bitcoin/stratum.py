...
class StratumRPCMiningProvider(object):
    ...
    __slots__ = ('pool_version_mask', 'wb', 'other', 'transport', 'authorized',
                 'username', 'handler_map', 'watch_id')
    ...
    def rpc_authorize(self, username, password):
        if not hasattr(self, 'authorized'): # authorize can be called many times in one connection
            print('>>>Authorize: %s from %s' % (
                username, self.transport.getPeer().host))
    ...
    def rpc_configure(self, extensions, extensionParameters):
        ...
        if 'version-rolling' in extensions:
            ...
            # according to the spec, pool should return largest mask
            # possible (to support mining proxies)
            return {"version-rolling" : True,
                    "version-rolling.mask" : '{:08x}'.format(
                        self.pool_version_mask & int(miner_mask, 16))}
            # pool can send mining.set_version_mask at any time if the
            # pool mask changes
    ...
    def _send_work(self):
        ...
        self.other.svc_mining.rpc_set_difficulty(
                bitcoin_data.target_to_difficulty(
                    x['share_target']) * self.wb.net.DUMB_SCRYPT_DIFF
                ).addErrback(lambda err: None)
        self.other.svc_mining.rpc_notify(
            jobid, # jobid
            getwork._swap4(pack.IntType(256).pack(
                x['previous_block'])).hex(), # prevhash
            x['coinb1'].hex(), # coinb1
            x['coinb2'].hex(), # coinb2
            [pack.IntType(256).pack(s).hex()
                for s in x['merkle_link']['branch']], # merkle_branch
            getwork._swap4(pack.IntType(32).pack(x['version'])).hex(), # version
            getwork._swap4(pack.IntType(32).pack(x['bits'].bits)).hex(), # nbits
            getwork._swap4(pack.IntType(32).pack(x['timestamp'])).hex(), # ntime
            True, # clean_jobs
        ).addErrback(lambda err: None)
        self.handler_map[jobid] = x, got_response

    def rpc_submit(self, worker_name, job_id, extranonce2, ntime, nonce,
                   version_bits = None, *args):
        # asicboost: version_bits is the version mask that the miner used
        worker_name = worker_name.strip()
        if job_id not in self.handler_map:
            print('''Couldn't link returned work's job id with its handler. This should only happen if this process was recently restarted!''',
                    file=sys.stdout)
            #self.other.svc_client.rpc_reconnect().addErrback(lambda err: None)
            return False
        x, got_response = self.handler_map[job_id]
        coinb_nonce = bytes.fromhex(extranonce2)
        ...
        header = dict(
            ...
            merkle_root=bitcoin_data.check_merkle_link(bitcoin_data.hash256(
                new_packed_gentx), x['merkle_link']), # new_packed_gentx has witness data stripped
            timestamp=pack.IntType(32).unpack(getwork._swap4(
                bytes.fromhex(ntime))),
            bits=x['bits'],
            nonce=pack.IntType(32).unpack(getwork._swap4(
                bytes.fromhex(nonce))),
        )
