...
class WorkerInterface(object):
    ...
    __slots__ = ('worker_bridge', 'worker_views', 'merkle_root_to_handler')
    ...
    def attach_to(self, res, get_handler=None):
        res.putChild(b'', _GETableServer(_Provider(self, long_poll=False),
                                        get_handler))

        def repost(request):
            request.content = StringIO.StringIO(json.dumps(
                dict(id=0, method='getwork')))
    ...
    def _getwork(self, request, data, long_poll):
        ...
        if request.getHeader('Host') is not None:
            request.setHeader('X-Stratum', 'stratum+tcp://%s' %
                    request.getHeader('Host'))

        if data is not None:
            header = getwork.decode_data(data)
            if header['merkle_root'] not in self.merkle_root_to_handler:
                print('''Couldn't link returned work's merkle root with its handler. This should only happen if this process was recently restarted!''',
                        file=sys.stderr)
                defer.returnValue(False)
            defer.returnValue(self.merkle_root_to_handler[header['merkle_root']](
                header, request.getUser() if request.getUser() is not None \
                        else '', '\0' * self.worker_bridge.COINBASE_NONCE_LENGTH))

        if p2pool.DEBUG:
            id = random.randrange(1000, 10000)
            print('POLL %i START is_long_poll=%r user_agent=%r user=%r' % (
                id, long_poll, request.getHeader('User-Agent'), request.getUser()))

        if long_poll:
            request_id = request.getClientIP(), request.getHeader('Authorization')
            if self.worker_views.get(request_id,
        ...
        res = getwork.BlockAttempt(
            ...
            merkle_root=bitcoin_data.check_merkle_link(
                bitcoin_data.hash256(
                    x['coinb1'] + '\0' * self.worker_bridge.COINBASE_NONCE_LENGTH + \
        ...
        if request.getHeader('User-Agent') == 'Jephis PIC Miner':
            ...
            extra_params = dict(identifier=str(
                self.worker_bridge.new_work_event.times), submitold=True)
        defer.returnValue(res.getwork(**extra_params))

class CachingWorkerBridge(object):

    __slots__ = ('_inner', 'net', 'COINBASE_NONCE_LENGTH', 'new_work_event',
                 'preprocess_request', '_my_bits', '_cache', '_times')
    ...
    def __init__(self, inner):
        ...
        self._my_bits = (self._inner.COINBASE_NONCE_LENGTH - \
                self.COINBASE_NONCE_LENGTH) * 8
    ...
    def get_work(self, *args):
        ...
        res = (
            ...
            lambda header, user, coinbase_nonce: handler(
                header, user, pack.IntType(self._my_bits
                    ).pack(nonce) + coinbase_nonce),
        )
