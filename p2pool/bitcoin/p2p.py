...
class Protocol(p2protocol.Protocol):
    ...
    def handle_version(self, version, services, time, addr_to, addr_from, nonce,
            sub_version_num, start_height):
        self.send_verack()
    ...
    def handle_headers(self, headers):
        for header in headers:
            ...
            self.get_block_header.got_response(bitcoin_data.hash256(
    ...
    def handle_reject(self, message, ccode, reason, data):
        if p2pool.DEBUG:
            print('Received reject message (%s): %s' % (message.decode('ascii'),
                reason.decode('ascii')),
                    file=sys.stderr)
    ...
    def connectionLost(self, reason):
        ...
        if hasattr(self, 'pinger'):
            self.pinger.stop()
        if p2pool.DEBUG:
            print('Bitcoin connection lost. Reason: %s' %
                    reason.getErrorMessage(), file=sys.stderr)

class ClientFactory(protocol.ReconnectingClientFactory):

    __slots__ = ('net', 'conn', 'new_block', 'new_tx', 'new_headers')
